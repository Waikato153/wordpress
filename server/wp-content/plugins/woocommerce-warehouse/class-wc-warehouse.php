...
require_once __DIR__ . '/class-wc-legacy-warehouse.php';

/**
 * Webhook class.
 */
class WC_Warehouse extends WC_Legacy_Warehouse {
	...
	public function __construct( $data = 0 ) {
		...
		if ( $data instanceof WC_Warehouse ) {
			$this->set_id( absint( $data->get_id() ) );
		} elseif ( is_numeric( $data ) ) {
			$this->set_id( $data );
		}

		$this->data_store = WW_Data_Store::load( 'warehouse' );
