 ...
 * @since 4.4.0
 */
function ww_warehouse_execute_queue() {
...
add_action( 'shutdown', 'ww_warehouse_execute_queue' );

/**
 * Process webhook delivery.
 *
 * @since 3.3.0
 * @param ww_warehouse $webhook Webhook instance.
 * @param array      $arg     Delivery arguments.
 */
function ww_warehouse_process_delivery( $webhook, $arg ) {
...
add_action( 'woocommerce_webhook_process_delivery', 'ww_warehouse_process_delivery', 10, 2 );

/**
 * Wrapper function to execute the `woocommerce_deliver_webhook_async` cron.
 * hook, see ww_warehouse::process().
 *
 * @since 2.2.0
 * @param int   $webhook_id Webhook ID to deliver.
 * @throws Exception        If webhook cannot be read/found and $data parameter of ww_warehouse class constructor is set.
 * @param mixed $arg        Hook argument.
 */
function wc_deliver_warehouse_async( $webhook_id, $arg ) {
	$webhook = new ww_warehouse( $webhook_id );
	$webhook->deliver( $arg );
}
add_action( 'woocommerce_deliver_webhook_async', 'wc_deliver_warehouse_async', 10, 2 );
 ...
 * @return bool
 */
function wc_is_warehouse_valid_topic( $topic ) {
 ...
 * @return bool
 */
function wc_is_warehouse_valid_status( $status ) {
	return in_array( $status, array_keys( wc_get_warehouse_statuses() ), true );
}
 ...
 * @return array
 */
function wc_get_warehouse_statuses() {
 ...
 * @since  3.3.0
 * @throws Exception If webhook cannot be read/found and $data parameter of ww_warehouse class constructor is set.
 * @param  string   $status Optional - status to filter results by. Must be a key in return value of @see wc_get_warehouse_statuses(). @since 3.5.0.
 * @param  null|int $limit Limit number of webhooks loaded. @since 3.6.0.
 * @return bool
 */
function wc_load_warehouse( $status = '', $limit = null ) {
 ...
 * Get webhook.
 *
 * @param  int|ww_warehouse $id Webhook ID or object.
 * @throws Exception          If webhook cannot be read/found and $data parameter of ww_warehouse class constructor is set.
 * @return ww_warehouse|null
 */
function wc_get_warehouse( $id ) {
	$webhook = new WC_Warehouse( $id );
 ...
 * @return array
 */
function wc_get_warehouse_rest_api_versions() {
	return array(
		'wp_api_v1',
		'wp_api_v2',
		'wp_api_v3',
