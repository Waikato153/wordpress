 ...
 */
class WW_Warehouse_Data_Store implements WW_Warehouse_Data_Store_Interface {
	...
	public function create( &$webhook ) {
		...
		$wpdb->insert( $wpdb->prefix . 'ww_warehouses', $data ); // WPCS: DB call ok.
	...
	public function read( &$webhook ) {
		...
		$data = wp_cache_get( $webhook->get_id(), 'warehouse' );



        $data = false;
		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT webhook_id, status, name, user_id, delivery_url, secret, topic, date_created, date_modified, api_version, failure_count, pending_delivery FROM {$wpdb->prefix}ww_warehouses WHERE webhook_id = %d LIMIT 1;", $webhook->get_id() ), ARRAY_A ); // WPCS: cache ok, DB call ok.
	...
	public function delete( &$webhook ) {
		...
		$this->delete_transients( 'all' );
		wp_cache_delete( $webhook->get_id(), 'warehouse' );
		WC_Cache_Helper::invalidate_cache_group( 'warehouse' );
		do_action( 'woocommerce_warehouse_deleted', $webhook->get_id(), $webhook );
	}
	...
	public function search_webhooks( $args ) {
		...
		// Check for cache.
		$cache_key   = WC_Cache_Helper::get_cache_prefix( 'warehouse' ) . 'search_webhooks' . md5( implode( ',', $args ) );
		$cache_value = false;//wp_cache_get( $cache_key, 'warehouse_search_results' );
		...
		if ( $args['paginate'] ) {
			$query = trim(
				...
				FROM {$wpdb->prefix}ww_warehouses
				WHERE 1=1
				{$status}
				{$search}
				{$include}
		...
		wp_cache_set( $cache_key, $return_value, 'warehouse_search_results' );
	...
	protected function get_webhook_count( $status = 'active' ) {
		global $wpdb;
		$cache_key = WC_Cache_Helper::get_cache_prefix( 'ww_warehouse' ) . $status . '_count';
		$count     = wp_cache_get( $cache_key, 'ww_warehouse' );

		if ( false === $count ) {
			$count = absint( $wpdb->get_var( $wpdb->prepare( "SELECT count( webhook_id ) FROM {$wpdb->prefix}ww_warehouses WHERE `status` = %s;", $status ) ) );

			wp_cache_add( $cache_key, $count, 'warehouse' );
		}
